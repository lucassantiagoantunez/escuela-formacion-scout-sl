...
def panel(request):
    ...
    certificados = Certificado.objects.filter(inscripcion__cursante=request.user, revocado=False)
    return render(request, 'aula/panel.html', {'cursos': cursos, 'pendientes': pendientes, 'certificados': certificados})
